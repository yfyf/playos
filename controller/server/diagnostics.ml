...
type interface_dict = (string * interface_info) list
[@@deriving protocol ~driver:(module Jsonm)]

let interface_dict_to_jsonm (dict : interface_dict) =
  `O (List.map (fun (k, v) -> (k, interface_info_to_jsonm v)) dict)

let interface_dict_of_jsonm _ = failwith "Expected a JSON object"
...
let ping_processes : (string, Lwt_process.process_none * float) Hashtbl.t =
  Hashtbl.create 16
...
let start_ping_process iface rate =
  ...
  if rate > 0.0 then (
    ...
    let proc =
      Lwt_process.open_process_none ~stdout:`Dev_null ~stderr:`Dev_null
        ( "ping"
        , [| "ping"
           ; "-I"
           ; iface
...
let cors_middleware =
  let filter handler req =
    ...
    | _ ->
        ...
        let headers =
          Cohttp.Header.add_list res.Response.headers cors_headers
        in
        Lwt.return { res with Response.headers }
  in
...
let build ~connman ~get_interface_annotations app =
  ...
  |> post "/diagnostics/wifi/on" (fun _ ->
         let%lwt _ =
           run_cmd_ignore
             "rfkill unblock wifi && for i in $(ls /sys/class/net | grep \
              '^w'); do ip link set $i up; done"
         in
         respond_ok ()
     )
  |> post "/diagnostics/wifi/off" (fun _ ->
         let%lwt _ =
           run_cmd_ignore
             "rfkill block wifi && for i in $(ls /sys/class/net | grep '^w'); \
              do ip link set $i down; done"
         in
         respond_ok ()
     )
  |> post "/diagnostics/eth/on" (fun _ ->
         let%lwt _ =
           run_cmd_ignore
             "for i in $(ls /sys/class/net | grep '^e'); do ip link set $i up; \
              done"
         in
         respond_ok ()
     )
  ...
  |> post "/diagnostics/ping/generate" (fun req ->
         ...
         | `O _ as json -> (
           ...
           | Error _ ->
               Lwt.return
                 (resp_json ~code:(`Code 400)
                    (Ezjsonm.dict
                       [ ("error", Ezjsonm.string "Invalid request") ]
                    )
                 )
         )
         | _ ->
             Lwt.return
               (resp_json ~code:(`Code 400)
                  (Ezjsonm.dict
                     [ ("error", Ezjsonm.string "Expected JSON object") ]
                  )
               )
         | exception _ ->
             Lwt.return
               (resp_json ~code:(`Code 400)
                  (Ezjsonm.dict [ ("error", Ezjsonm.string "Invalid JSON") ])
               )
     )
