...
The page shows the status of all network interfaces and the list of available
wireless networks.

#+CAPTION: Network
#+NAME: fig:controller-network-list
#+attr_html: :width 800px
[[../screenshots/controller-network-list.png]]

- The _Internet_ indicator shows whether the internet connection is functional.
- The ~Interfaces~ section provides an overview of each network interface's
  status.
- Each interface is marked as connected (✔) or disconnected (✖).
- If an interface is connected, the connected network is indicated as /Wired/
  (for Ethernet interfaces) or using the wireless network name (for Wireless
  interfaces).
- Interfaces might also receive additional annotations via mDNS, which are
  also displayed in ~Connected networks~ (e.g. /Dividat Senso/ in the
  screenshot, annotating a link-local device connection).

Note: the page does not automatically refresh, you must manually press /Refresh/
to obtain up to date information.

To connect to a wireless network, select it from the list of available networks
and enter the password, if any:
...
A connected interface or wireless service may be selected for additional configuration. Optional proxy servers and static IP addresses may be configured:
